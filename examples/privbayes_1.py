...
def worker(data, eps, seed):
    print("7.4. back to main func loaded successfully")
    ...
    print("7.5. back to main func loaded successfully")

    values = np.ascontiguousarray(data.df.values.astype(np.int32))
    print("7.6. back to main func loaded successfully")

    ans = privBayesSelect.py_get_model(values, config, eps / 2, 1.0, seed)
    print("7.7. back to main func loaded successfully")
    ...
    print("7.8. next is measuremernt")
    measurements = []
    print("7.9. measuremrent is over")
...
def get_dataset_file(dataset_name):
    dataset_folder = '/Privbayes/private-pgm/data/'
...
def preprocess(original_dataset):
    ...
    output_directory = '/Privbayes/private-pgm/data/'
...
def privbayes_measurements(data, eps=1.0, seed=0):
    ...
    print("7.1. back to main func loaded successfully")

    results = []
    for _ in range(num_processes):
        results.append(pool.apply_async(worker, args=(data, eps, seed)))

    pool.close()
    pool.join()
    print("7.2. back to main func loaded successfully")

    final_measurements = []
    for res in results:
        measurements = res.get()
        final_measurements.extend(measurements)
    print("7.3. back to main func loaded successfully")
...
def privbayes_inference(domain, measurements, total, file_name):
    ...
    output_folder = '/Privbayes/private-pgm/data/output/'  # Define the output folder path
...
if __name__ == '__main__':
    ...
    print("8. measurement function is done")
    engine = FactoredInference(data.domain, backend='torch', log=True)
